...
/**
 * The loader script every press embed snippet points at.
 *
 ...
 *
 * A separate route from `/embed/review-widget.js` rather than one script that
 * scans for both attributes. A customer running only the review widget should
 * not be shipping the press widget's code to every visitor, and a page running
 * both simply loads two ~2 KB files that share an origin and ignore each
 * other's messages. `buildLoaderScript` is the single implementation.
 *
 ...
 * it can never carry a build hash. That constrains caching: a long `max-age`
 * would leave a fixed bug live in browsers for its duration, so the browser
 * cache is short and the shared cache does the heavy lifting with a long stale
 * window.
 */
...
export async function GET(): Promise<Response> {
  return new Response(buildLoaderScript(appOrigin(), "press"), {
    headers: {
