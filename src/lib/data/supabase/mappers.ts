...
export function toMention(row: Row): Mention {
  return parseOrThrow(
    mentionSchema,
    {
      ...
      conversationRootExternalId: row.conversation_root_external_id ?? null,
      sourceCommunity: row.source_community ?? null,
      sourceScore: num(row.source_score),
      sourceCommentCount: num(row.source_comment_count),
      sourceIsLocked: row.source_is_locked ?? false,
      sourceIsArchived: row.source_is_archived ?? false,
      sourceIsNsfw: row.source_is_nsfw ?? false,
      sourceRemovedAt: isoOrNull(row.source_removed_at),
      sourceLastVerifiedAt: isoOrNull(row.source_last_verified_at),
      createdAt: iso(row.created_at),
      updatedAt: iso(row.updated_at),
    },
    "mention",
