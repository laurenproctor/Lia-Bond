-- Audit vocabulary for Stripe subscription billing. Sixteen billing.* names covering the checkout, trial, subscription, capacity, payment, and operator-grant lifecycles, plus the organization_billing entity type added by 20260821000400. Regenerated after merging origin/master so it carries the press widget's names as well -- the branch's first draft predated them and, sorting last, would have dropped every press_widget.* event.
--
